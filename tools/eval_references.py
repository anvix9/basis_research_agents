"""
ARANEA Evaluator — Reference Hallucination Checker
---------------------------------------------------
...
def search_semantic_scholar(title: str, author: str = "") -> dict | None:
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "ARANEAEvaluator/1.0"},
            timeout=15,
        )
...
def generate_report(run_id: str, results: list[VerifyResult], problem: str) -> str:
    ...
    lines += [
        "---",
        "",
        f"*Report generated by ARANEA Evaluator v1.0 — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}*",
...
def main():
    parser = argparse.ArgumentParser(description="ARANEA Reference Hallucination Evaluator")
    ...
    print(f"\n{'='*60}")
    print(f"  ARANEA Reference Hallucination Evaluator")
    print(f"{'='*60}")
