"""
ARANEA Evaluator — Claim Verification
--------------------------------------
...
def generate_report(run_id: str, results: list[ClaimResult], problem: str) -> str:
    ...
    lines += [
        "---",
        "",
        f"*Report generated by ARANEA Claim Evaluator v1.0 — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}*",
...
def main():
    parser = argparse.ArgumentParser(description="ARANEA Claim Verification Evaluator")
    ...
    print(f"\n{'='*60}")
    print(f"  ARANEA Claim Verification Evaluator")
    print(f"{'='*60}")
